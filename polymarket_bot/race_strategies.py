...
def _simple_exit_plan(position: dict[str, Any], current_pnl_pct: float, settings: Settings) -> dict[str, Any] | None:
    ...
    # Ported from main: the loss must persist for _SL_CONFIRM_TICKS CONSECUTIVE
    # ticks before we sell, so a one-tick thin-book phantom bid can't trigger
    # it. Scope (user rule): ONLY team-win Yes/No bets get the SL. All other
    # markets — O/U 4.5 totals, anything non-moneyline — NEVER stop out.
...
def _run_race_tick(
    ...
    for candidate in picks:
        ...
        # Applies to top-ups too: a phantom ask must not grow a position.
        ...
        # Tiered stake by entry price (kzer_windows): high-confidence zone
        # (0.92+) scales × 1.10, lower zone (0.88–0.91) × 0.70 — but the
        # tier can never pierce the per-bet cap (equity × race_stake_pct).
        _entry_price = float(candidate.best_ask or candidate.price or 0.0)
        _tier_mult = 1.10 if _entry_price >= 0.92 else 0.70
        target = min(target * _tier_mult, equity * settings.race_stake_pct)
