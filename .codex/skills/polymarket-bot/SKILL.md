...
- **Entry:** all three live profiles admit asks ∈ [0.85, 0.97], close within 6h, target the city's local current date, and wait until solar 15:00. Weather scans also inspect API deadlines from the prior 24h because contracts can remain orderable after Gamma's noon-UTC `endDate`. Range brackets are enabled, and distinct brackets from the same city/date event may each be held (exact market/token dedup remains). Model/calibration and region/date entry safeguards are disabled.
