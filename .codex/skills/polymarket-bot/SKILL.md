...
- **Entry:** all three live profiles admit asks ∈ [0.85, 0.97], close within 6h, target the city's local current date, and wait until solar 15:00. Weather scans also inspect API deadlines from the prior 24h because contracts can remain orderable after Gamma's noon-UTC `endDate`. Range brackets are enabled; forecast-gated bots retain their bracket-margin protection. Region/date exposure is capped at two lines. Open-Meteo is fail-closed and a 30-sample Brier gate pauses entries above 0.12.
