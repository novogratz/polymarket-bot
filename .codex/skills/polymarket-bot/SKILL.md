...
- **Live strategy:** `grinder` — race mode, **WEATHER-ONLY** (2026-07-06): `weather_only = true` keeps ONLY weather / temperature markets (`is_weather_market`); everything else is dropped at selection. All 3 bots. Bots 2 and 3 now share the same production strategy: 0.80 entry floor with the optional Open-Meteo edge and bracket-margin gates disabled. Bot 1 retains the stricter 0.85 floor and 0.10 / 2°C forecast gates.
