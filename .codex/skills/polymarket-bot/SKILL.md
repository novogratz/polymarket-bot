...
- **Entry:** all three live profiles admit asks ∈ [0.85, 0.97] with a matching 0.97 hard cap and a +0.01 forecast-edge gate (0.97 therefore requires model probability ≥0.98); each scans 1,500 markets per ordering, admits $50+ daily volume, and has no broad region/date count ceiling. Markets resolve today or in 12–24h (never beyond 24h), spread ≤4¢, narrow `between X–Y°` brackets stay excluded, and Open-Meteo remains fail-closed.
