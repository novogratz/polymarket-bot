...
- **Resolved-exit now reads the LIVE order book — stale quotes held winners past 0.99**: the exit decision used Gamma's flipped market-level quote and the data-API `curPrice`, both of which lag the CLOB near resolution. Seen live 2026-06-10 on all three bots: Israel-airspace No had a real 0.99 book bid while the exit loop saw 0.95 (Iran-airspace: 0.99 real vs 0.98 seen) and the winners were never sold. `_execute_race_exits` now probes the live CLOB best bid per open position (`live_best_bid` in `trading.py`, fail-open to the cached price when the book is unavailable) and decides + prices the sell off it. In the same change `resolved_exit_threshold` drops 0.99 → **0.98** in both profiles (user rule 2026-06-10: "if 0.99 doesn't work just close at 0.98") — the sell is a marketable limit at min(bid, 0.99), so a 0.99 bid still fills at 0.99 and a 0.98 bid closes the bet at 0.98 instead of waiting for a print that may never come. Regression tests in `tests/test_strategy.py` (live-bid override, 0.98 fill, fail-open probe).
