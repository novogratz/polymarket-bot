...
- **RAPPORT LIVE — open positions sorted by expiry + end time shown** (user request 2026-06-11): `POSITIONS OUVERTES` now lists positions soonest-to-resolve first (positions without a known end date last) and each line shows when the game finishes / the market expires — `⏳ Fin prévue : 14:05 ET (dans 2h05)`, with a date prefix when it isn't today and `⌛ Échéance passée — résolution en cours` once the end time is behind us. End dates come from the Data API `endDate` with a local-ledger fallback. Wording stays an estimate because Gamma's endDate is often set before kickoff for sports.
