...
- **Stock market / equities — ONGOING SESSION ONLY, ask ≥ 0.90 (2026-06-12)**: tradeable only Mon–Fri 09:30–16:00 ET, only for that day's close (`endDate` within 12 h), and only at ask ≥ **0.90** (`STOCK_MIN_ASK`). Overnight/weekend/multi-day stay excluded, and **weekly ranges / intraday touch markets ("Week of", "hit (LOW)/(HIGH)") are banned outright** — no end-of-session convergence (the ABNB weekly slipped in 2026-06-11). Patterns: indices & ETFs, big-cap tickers & companies (word-bounded `_STOCK_MARKET_RE` incl. ABNB/Airbnb, UBER, COIN, PLTR, HOOD), equity terms, `closes above/below $X`, plus the generic `(TICKER) … $` rule
