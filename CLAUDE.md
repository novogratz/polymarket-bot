...
- price (ask) ∈ **[0.85, 0.97]**, **dynamic entry window**: ≤ **4 h** to close preferred; when no candidate is actionable the window widens 4 → 6 → 8 → 10 and **stops at 12 h — nothing beyond 12 h, ever** (`race_max_hours=4`, `race_max_hours_cap=12`, `race_daily_expiry_fallback=0`; user 2026-06-12 after the 24 h/daily rungs bought overnight holds twice; a by-tomorrow market becomes tradeable only once within 12 h of its end; ladder in `_entry_window_ladder`). The Gamma load always covers the widest rung so held positions keep being marked and managed.
...
- **Winner floor (0.99)** — `execute_live_sell` refuses any `race_big_win_resolved`/`resolved_market_sweep_win` order priced below 0.99 (the position holds instead), `_sweep_sell_live` clamps to exactly 0.99, and the self-tuner's `resolved_exit_threshold` bounds are pinned to (0.99, 0.99). Resolved winners sell at 0.99 or settle at 1.00 — never 0.97/0.98.
...
- **Stock market / equities — ONGOING SESSION ONLY (2026-06-12, was a blanket ban)**: tradeable only Mon–Fri 09:30–16:00 ET *and* only for that day's close (`endDate` within 12 h). Overnight/weekend/multi-day stay excluded, and **weekly ranges / intraday touch markets ("Week of", "hit (LOW)/(HIGH)") are banned outright** — no end-of-session convergence (the ABNB weekly slipped in 2026-06-11). Patterns: indices & ETFs, big-cap tickers & companies (word-bounded `_STOCK_MARKET_RE` incl. ABNB/Airbnb, UBER, COIN, PLTR, HOOD), equity terms, `closes above/below $X`, plus the generic `(TICKER) … $` rule
...
- **Esports — LIVE GAMES ONLY (2026-06-12, was a blanket ban)**: tradeable only while the game is in progress (`gameStartTime` in the past, ≤ 8 h). Pre-game or unknown start stays excluded. Patterns: counter-strike/valorant/league of legends/"LoL:"/dota/cs2/csgo/rainbow six/rocket league/overwatch + BO1/BO3/BO5
...
2. Build eligible candidates (entry filters + exclusions); log a wide forward-observation net. Entries use the narrowest ladder window (4 → 6 → 8 → 10 → 12 h max) that has actionable candidates; same-game picks collapse to one (soccer: under 4.5 preferred).
