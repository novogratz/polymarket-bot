...
**Sizing (5% FIXED-FRACTION, NO REINFORCEMENT — user 2026-07-11 THE RULE):** `full_deploy = true` + `full_deploy_max_position_pct = 0.05` in BOTH profiles. Every NEW position stakes **exactly 5% of equity** (`_full_deploy_cap_usd`, $200 → $10, floored at $5 for Polymarket's minimum; `_dynamic_stake_target` returns the cap itself — NOT cash/N — so 40 eligible markets still means 5% each and the bankroll deploys across up to 20 distinct lines). **A held market is NEVER bought again** ("you cant rebet on a position that is already existing"): held tokens are dropped from the pick slots in `_actionable_candidates` — no top-up, no redistribution, no double-down, no re-bet. Cash that can't find a NEW market waits. The 3-tick equal redistribution (#121) was REMOVED 2026-07-11 — with a 10 s tick it fired after ~30 s and pumped single lines (the $33 Paris bug). `cash_floor_pct = 0`. Worst-case loss on one line ≈ 5% of equity. `full_deploy` OVERRIDES `race_fixed_stake_usd`; rollback to fixed-$5 = `full_deploy = false`, `fixed_stake_usd = 5.0`; `full_deploy_max_position_pct = 0` = legacy uncapped cash/N. Legacy `stake_pct`/`initial_stake_pct` stay ignored; dip double-down stays DISABLED. **ON-CHAIN no-rebet guard (2026-07-11):** `execute_live_trade` refuses any live BUY on a token the wallet already holds ≥1 share of (`rebet_blocked`, chain-checked via `live_share_balance`, fail-open on probe errors, OFF in legacy mode) — the no-reinforce rule holds even when the local ledger lies. `FullDeploySizingTests` + `NoRebetGuardTests` pin the behavior. See `docs/STRATEGIES.md`.
