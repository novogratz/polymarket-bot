...
- price (ask) ∈ **[0.85, 0.97]**, **dynamic entry window**: ≤ **4 h** to close preferred; when no candidate is actionable the window widens 4 → 6 → 8 → 10 → 12, jumps to **24 h**, and **stops — nothing beyond 24 h, ever** (`race_max_hours=4`, `race_max_hours_cap=24`, `race_daily_expiry_fallback=0` — the end-of-tomorrow rung was reverted the day it shipped after it bought ~36 h overnight holds; ladder in `_entry_window_ladder`). The Gamma load always covers the widest rung so held positions keep being marked and managed.
...
2. Build eligible candidates (entry filters + exclusions); log a wide forward-observation net. Entries use the narrowest ladder window (4 → 6 → 8 → 10 → 12 → 24 h max) that has actionable candidates; same-game picks collapse to one (soccer: under 4.5 preferred).
