...
**Dip double-down (2026-06-14):** when ANY held position's LIVE ask dips 1–8¢ below its entry — e.g. 0.96 → 0.89 (`_execute_double_downs`) — the bot buys more of the same outcome, averaging the cost basis down on a favorite that got cheaper. Once per position (`doubled_down` flag), only while the ask stays in the price band [min,max], never past the 10% per-bet cap. A bigger adverse move past 8¢ → no add (never catch a falling knife). Off unless `race_double_down_enabled`.
