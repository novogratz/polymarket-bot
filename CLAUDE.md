...
- **Per-lane entry floors** (above the global 0.85): **esports ≥ 0.92** (`ESPORTS_MIN_ASK`) and **soccer/sport "Will <X> win on <date>?" moneylines ≥ 0.92** (`SOCCER_MONEYLINE_MIN_ASK`, user 2026-06-17). Moneylines gap catastrophically on a single goal: EVERY moneyline loss in the realized history entered at ≤ 0.90 (0.85, 0.86, 0.867, 0.895, 0.90); the 0.90+ band has ZERO losses across 29 trades. The SL can't protect a goal-gap (Difaâ "No" 0.89 → 0.02), so the control is at entry — both Yes and No sides of such a market are floored (`_is_soccer_moneyline_text`).
...
- The scan reads the **top 100 markets** per ordering (`scan_limit=100`, user 2026-06-15 — a single Gamma page, no pagination); held/pending/capped markets are removed **before** the top-4 pick truncation so they never burn slots.
...
**Sizing (Kelly, dynamic — user 2026-06-18 "10% isn't enough"):** the binding lever is the **entry size**, not the cap — most winners never dip, so they ride at `initial_stake_pct`, and the cap is only reached via the dip double-down. **Fresh entries open at `initial_stake_pct = 0.20` (20% of equity)** and the double-down fills the reserved headroom up to the **hard cap `stake_pct = 0.35`** (`_entry_cap_usd` vs `_position_cap_usd`). This is **near-full-Kelly, aggressive by explicit choice**: for win rate p≈0.97, gain b≈8.4%, worst-case total loss a≈1.0, two-point Kelly `f* = (p·b − q·a)/(a·b) ≈ 0.35`. Worst single total loss is −20% (entry) / −35% (doubled-down). With `initial_stake_pct = 0` entries target the full cap; the per-bet target spreads available cash across the actionable opportunities (cash/N), so a busy window funds every market and a slow market gives each bet the full cap. The <30 min/<1 h boost scales the spread share but never pierces the cap (`_dynamic_stake_target`). Scales automatically with the bankroll. The self-tuner may move `stake_pct` within (0.05, 0.35). See `docs/STRATEGIES.md` for the full Kelly derivation.
...
- **Fed / FOMC rate-decision markets banned (2026-06-17)** — `fed rate`, `rate cut`/`rate hike`, `cut/hike/raise rates`, `interest rate`, `fomc`, `fed funds`, `rate decision`, `basis points`. "Fed rate cut by September 2026 meeting?" resolves months out but shows a near-term Gamma `endDate`, slipping through the 4 h window — the bots re-bought it repeatedly after manual sells. Monetary-policy-specific phrasing so "approval rating" etc. is unaffected
