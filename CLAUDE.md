...
**Sizing (dynamic):** hard cap **15% of equity per bet** (`stake_pct = 0.15`; raised from 10% 2026-06-14 to size up winners at the 89% win rate). **Fresh entries open at the lower `initial_stake_pct` (5%, 2026-06-14)** and the dip double-down fills the reserved headroom up to the 10% cap — so the double-down always has room to add (`_entry_cap_usd` vs `_position_cap_usd`). With `initial_stake_pct = 0` entries target the full cap (old behavior); the per-bet target spreads available cash across the actionable opportunities (cash/N), so a busy window funds every market and a slow market gives each bet the full 10%. The <30 min/<1 h boost scales the spread share but never pierces the cap (`_dynamic_stake_target`). Depth-capped entries can be topped up later toward the same 10% cap. Scales automatically with the bankroll.
