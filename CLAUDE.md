...
- The scan reads the **top 100 markets** per ordering (`scan_limit=100`, user 2026-06-15 — a single Gamma page, no pagination); held/pending/capped markets are removed **before** the pick truncation (`max_orders_per_tick = 12` in v4 — maximize bets per tick) so they never burn slots.
