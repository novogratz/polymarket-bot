...
**Sizing (v4 fixed-dollar — user 2026-06-21 "Polymarket Bot v4"):** every trade stakes **EXACTLY $5** (`race_fixed_stake_usd = 5.0`) — no Kelly, no % of equity, no martingale, no averaging-down, no double-down, no confidence scaling, no dynamic spread. When `race_fixed_stake_usd > 0` the three sizing functions (`_position_cap_usd`, `_entry_cap_usd`, `_dynamic_stake_target`) all return the flat amount (capped only by available cash), so worst single-trade loss is $5 and the bankroll fully deploys across `bankroll / 5` positions ($50 → 10, $500 → 100). The legacy `stake_pct`/`initial_stake_pct` Kelly knobs are IGNORED while fixed sizing is on (kept for the legacy % mode; the self-tuner still bounds `stake_pct` for that path). **Dip double-down is DISABLED** (`race_double_down_enabled = false`) — fixed sizing means no averaging into a position. See `docs/STRATEGIES.md`.
...
**Excluded markets** (`models.py:is_excluded_market`, across every lane). **v4 (user 2026-06-21): `unban_all_markets = true` BYPASSES this entire list at entry selection** — every category is allowed and governed instead by the **data-driven category auto-disable** (`categories.py`): after ≥ `race_category_min_samples` (100) realized trades in a category, it is dropped from entry selection if its ROI < `race_category_disable_roi` (−5%). `_run_race_tick` computes the disabled set from the realized ledger each tick (fail-open) and `_build_eligible_candidates` filters on it. Forward-looking — a fresh bot disables nothing; `other` is never auto-disabled. Per-trade risk is bounded by the $5 fixed stake.
- **Crypto is ALWAYS banned, even under `unban_all_markets`** (user 2026-06-24, "ban crypto for all bots 1 2 3"). `_build_eligible_candidates` calls `models.is_crypto_market` as an always-on filter (like the resolution-clarity gate) that ignores the unban flag — crypto never reaches candidate selection on any bot. Crypto Up/Down binaries have no ride-to-resolution convergence edge and were a top loss category. Pinned by `test_crypto_banned_even_under_unban_all`.

The ban list below is what applies when `unban_all_markets = false` (crypto, first item, applies regardless):
- **All crypto** — bitcoin/btc/ethereum/solana/dogecoin/xrp/cardano/litecoin/"crypto" + Up/Down binaries (now banned **always**, see above — not just under `unban_all_markets = false`)
...
- **v4 performance** (≥10 closed trades): `PERFORMANCE v4 :` ROI / Sharpe / profit factor / max drawdown; a **p/q/edge** line for **all-time and today** (`🎯 p=avg entry price · q=win rate · edge(q−p) in pts` — the bot is +EV only when q>p, user 2026-06-22); a **best/worst category** line (`🥇 Meilleure catégorie … 🥶 Pire …` ranked by realized $ P&L, ROI + trade count alongside, user 2026-06-23); plus `🏷️ Catégories à risque :` (worst per-category ROIs, ⛔ on auto-disabled ones) — the data-driven governance at a glance (`_v4_performance_lines` / `_pq_line`)
