...
| `full_deploy` | bool | **Sizing FRACTION FIXE 5% (ACTIF, 2026-07-11).** `true` → chaque NOUVELLE position mise exactement `full_deploy_max_position_pct` de l'équité ; une ligne détenue n'est JAMAIS rachetée (pas de top-up / redistribution / re-bet) ; le cash sans nouveau marché attend. OVERRIDE `fixed_stake_usd`. Env var : `POLYMARKET_RACE_FULL_DEPLOY`. |
| `full_deploy_max_position_pct` | float | **LA RÈGLE (2026-07-11) : fraction d'équité de chaque nouvelle position** (défaut profils **0.05** = 5%, plancher $5 pour le minimum Polymarket). Le stake de chaque nouvelle position EST ce cap (fraction fixe) ; le bankroll se déploie sur jusqu'à 20 lignes distinctes ; le cash sans nouveau marché attend. 0 = legacy cash/N sans cap. Env var : `POLYMARKET_RACE_FULL_DEPLOY_MAX_POSITION_PCT`. |
