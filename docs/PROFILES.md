...
| `full_deploy_max_position_pct` | float | **Cap de diversification (2026-07-10, « positions at $90 when bankroll total is $200 is not acceptable »).** Aucune position ne peut dépasser cette fraction de l'équité (défaut profils **0.10** = 10%, plancher $5 pour le minimum Polymarket). Le bankroll se répartit sur ≥10 marchés météo distincts quand ils existent ; le cash que le cap ne peut pas placer attend de nouveaux marchés au lieu de s'empiler. 0 = sans cap (comportement 2026-07-09). Env var : `POLYMARKET_RACE_FULL_DEPLOY_MAX_POSITION_PCT`. |
