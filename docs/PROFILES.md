...
| `tweet_only` | bool | **Lane TWEET-COUNT (proposée 2026-07-27, branch `feat/tweet-count-lane` — remplacement bot 2, pas encore live).** `true` → la sélection ne garde QUE les marchés de comptage de tweets/posts adossés à xtracker (`is_tweet_market` : "tweet" dans la question, slugs `-tweets`/`of-tweets`) et lève leur hard ban. Composable avec `weather_only` (union des deux univers). Défaut `false`. Env var : `POLYMARKET_RACE_TWEET_ONLY`. |
| `tweet_min_edge` | float | Gate d'edge du modèle de comptage (opt-in, 0 = off). N'entre que si `model_P(outcome) − ask ≥` ce seuil — modèle Poisson non-homogène heure-de-semaine + facteur burst 72h + queues negative-binomial, fitté sur le feed xtracker qui RÉSOUT le marché (`polymarket_bot/tweet_model.py`). Une fenêtre non matchable sur xtracker est SKIP (jamais devinée). `tweet_b.toml` : `0.08`. Env var : `POLYMARKET_RACE_TWEET_MIN_EDGE`. |
