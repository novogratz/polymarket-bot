...
Every Gamma market closing within the 48 h window (hard max — 4 h → 24 h for the weather-only lane, → 48 h on 2026-07-19 to include tomorrow's markets and double the simultaneous lines), fetched with two orderings (soonest-closing + highest-volume) and **full pagination past the API's silent 100-row cap** — ~1,000–2,000+ raw markets per tick depending on time of day.
...
| Entry window | **game starts OR market closes within ≤ 48 h** (2026-07-19 — today's + tomorrow's weather; was 24 h); dynamic widening disabled (`max_hours_cap = 0`) |
