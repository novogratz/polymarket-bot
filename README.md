...
Both scripts read `configs/profiles/claude_high_vol_panic.toml` as the live config. Run in the foreground; `Ctrl+C` to stop the whole process group.
...
The complete list lives in `polymarket_bot/config.py`. The canonical source of truth for live config is `configs/profiles/claude_high_vol_panic.toml`, loaded by `scripts/run_live_70.sh` (or `scripts/run_all.sh`).
