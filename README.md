...
## Commande live recommandee (~$70 bankroll)

Pour eviter les soucis de copy/paste avec les backslashes en zsh, utilise le script :

```bash
bash scripts/run_live_70.sh
```

Ou lance la commande sur une seule ligne :
...
- `POLYMARKET_MIN_OPEN_POSITIONS=3` : le bot tente toujours de remonter au moins jusqu'a 3 positions ouvertes en activant le scan de fallback (consensus relaxe) si le scan strict ne suffit pas. Les filtres de qualite (spread, prix, fraicheur) restent actifs : si rien ne passe les filtres, le bot ne force pas un trade.
- `POLYMARKET_SMART_STOP_LOSS_PCT=0.40` : sortie automatique a -40 % (apres `STOP_LOSS_MIN_AGE_MINUTES=15`).
- `POLYMARKET_SMART_COHORT_EXIT_ENABLED=1` : sortie si aucun wallet du cohort d'entree ne rachete le token dans la fenetre de lookback.
- `POLYMARKET_SMART_MAX_RELATIVE_SPREAD=0.30` : rejette les marches ou spread/ask > 30 %.
...
Commande live recommandee actuelle :
