...
### 5. Sizing (FULL-DEPLOY + diversification cap — user 2026-07-09/10)
...
- **Diversification cap (2026-07-10):** no single position may exceed **`full_deploy_max_position_pct` = 10% of equity** (floored at $5 for Polymarket's minimum; 0 = uncapped). A $90 position on a $200 bankroll is impossible — the bankroll spreads across ≥10 distinct weather markets (different cities) when they exist. Cash the cap can't place waits for new markets instead of piling onto one; diversification wins over strict 100% deployment.
- Leftover cash (e.g. from a depth-capped FOK fill) keeps flowing into already-held markets via the top-up lane — up to the cap, never past it. Every top-up re-passes every entry filter.
