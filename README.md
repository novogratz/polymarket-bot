...
The engine supports more than one strategy off the same pipeline; each live bot is a TOML profile pointing the engine at a candidate set and edge model. **All 3 bots now run a deterministic category-aware late-resolution lane:** weather, sports underway for at least 30 minutes, sub-hour windowed BTC/SOL/XRP crypto, and objective scheduled economics within two hours. Politics, entertainment, esports, speech, equities, daily crypto and ETH direction markets remain excluded.
