#!/usr/bin/env bash
# BOT 2 ONLY — LIVE avec le profil grinder_b2 (filtres assouplis, voir le TOML).
...
# MUST match grinder_b2.toml starting_cash — a mismatched fallback skews the
...
export POLYMARKET_PROFILE_LABEL=grinder_b2
...
POLYMARKET_QUIET=1 \
    ...
    uv run pmbot auto-loop --dry-run --profile grinder_b2 --run grinder_b2 \
...
uv run pmbot auto-loop --live --profile grinder_b2 --yes \
