...
class LoadProfileTests(unittest.TestCase):
    ...
    def test_all_live_grinders_share_category_aware_late_universe(self):
        ...
        for name in ("grinder.toml", "grinder_b.toml", "grinder_c.toml"):
            with self.subTest(profile=name):
                ...
                self.assertEqual(profile.values["POLYMARKET_RACE_WEATHER_STOP_PRICE"], "0.0")
                self.assertEqual(profile.values["POLYMARKET_RACE_SL_PCT"], "1.0")
                self.assertEqual(profile.values["POLYMARKET_RACE_WEATHER_FLIP_EXIT_PROB"], "0.0")
                self.assertEqual(profile.values["POLYMARKET_RACE_WEATHER_MIN_SOLAR_HOUR"], "0.0")
                self.assertEqual(profile.values["POLYMARKET_RACE_MAX_SPREAD"], "1.0")
                ...
                self.assertEqual(profile.values["POLYMARKET_RACE_WEATHER_CITY_DATE_CAP"], "2")
